...
class AuthControl extends CI_Controller{
	...
	public function verification_email_resend(){
		...
		$temp_id = UNIQUE_ID.mt_rand(1,9999999);
	...
	public function forgot_password(){
		setSession("otp_generated",true);
		$this->load->view('back/auth/forgot_password');
	}

	public function reset_password(){
		if(getSession("otp_generated")){
			$user = $this->SettingModel->get_record("bee_users",array("email"=>$_POST['username']),false);

			$template_content = file_get_contents(base_url('assets/templates/forgot_password.html'));
			...
			setFlashdata("otp",$otp);
			setFlashdata("user_id",$user['user_id']);
			$content = str_replace($from,$to,$template_content);
		...
		send_mail("sd",array(
			...
			$this->load->view('back/auth/reset_password');
		}
		else{
			$this->load->view('back/auth/session_expired');
		}

	}

	public function forgot_password_check(){
		setSession("otp_generated",false);
		if($_POST['password']===$_POST['cnf_password']){
			$data = array(
				"password"=>md5($_POST['password'])
			);

			$res = $this->SettingModel->update_record("bee_auth_users",$data,array("user_id"=>getSession('user_id')));

			r_success("","Password changed successfully");
		}
		else{
			r_fail("","Password or confirm password is not same!");
		}
	}
}
